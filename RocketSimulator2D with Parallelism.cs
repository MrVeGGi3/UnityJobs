...
//Matheus Veras e Daniel Marto - 5° JDM
using System;
...
class Program
{
    static void Main()
    {
        ...
        Parallel.ForEach(rockets, rocket =>
        {
            definirEmpuxoAtual(rocket, planetas[0]); //define empuxo máximo que é a força que faz o foguete subir
        });
    
        while (time < 10 && rockets[0].posY < planetas[0].raio)//enquanto o tempo for menor que "x" segundos e o foguete não sair da órbita do planeta
        {
            ...
            AtualizarStatus(rockets[0], planetas[0])
            Parallel.Invoke(
                () => TimeCount(),
                () => ExecutarMúsica()
            );
            if (rockets[0].posY >= planetas[0].raio)//se a posição do foguete for maior que o raio da terra
            {
                Console.WriteLine("O Foguete saiu do Planeta com sucesso!");
            }
            definirRazaoMassa(rockets[0]);//Fazer a atualização da quantidade de massa do foguete em cada segundo
            definirEmpuxoAtual(rockets[0], planetas[0]);//Fazer a atualização do empuxo do foguete (porque a mudança de massa influencia no empuxo)
            Console.WriteLine();
    ...
    static void definirEmpuxoAtual(Rocket rocket, Planeta planeta)
    {
